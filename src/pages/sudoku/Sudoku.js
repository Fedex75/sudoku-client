...
const Sudoku = ({theme}) => {
	...
	function handleKeyPress(e){
		if (e.key === 'Enter') {
			...
			if (['1', '2', '3', '4', '5', '6', '7', '8', '9'].includes(e.key) && possibleValuesRef.current.includes(Number.parseInt(e.key))){
				...
				GameHandler.game.fullNotation = !GameHandler.game.fullNotation
	...
	useEffect(() => {
		...
		const windowUnloadEvent = window.addEventListener('unload', () => {
			...
			<SectionContent paddingTop={0}>
				{
					...
					<div className="game">
						<div className="sudoku">
							<Canvas onClick={onClick} showLinks={showLinks} game={GameHandler.game} lockedInput={lockedInputRef.current} theme={theme} setAnimationCallback={cb => {animationCallback = cb;}} />
						</div>
